---
title: "인허가 문서 작성 AI Agent 설계: SoT와 Context 구조를 통한 사업화 가능성 탐색"
...
![Source of Truth and Context](images/source-of-truth-context/cover.png)

### Source of Truth (SoT)

Source of Truth는 여러 문서에 반복 사용되는 기준 데이터다.
...
Context는 중요하지만, 곧바로 canonical truth는 아니다.

---

## AI Agent의 문서 작성 방식

AI Agent는 SoT를 직접 재작성하지 않고, 스크립트와 Context를 사용해 문서 수정안을 만든다.
...
예전에도 사람은 SoT와 Context를 가지고 있었다.
...
- After  
  SoT와 Context를 AI Agent가 읽고 추적할 수 있는 형식으로 입력한다.
...
SoT는 YAML로 만들고, Context는 registry로 관리한다.

![Current implementation structure](images/current-implementation-structure/cover.png)

- SoT YAML  
  requirements, architecture, tests, risk controls, AI models

- Context Registry  
  MFDS guides, existing docs, review notes, working notes

- Typst + Scripts  
  YAML SoT를 문서로 렌더링하고 validate, trace, build를 수행한다.
...
- YAML 데이터를 읽어 SoT를 문서에 탑재

---

## Typst에서 SoT를 렌더링한다

SoT를 복사해 붙이는 대신 Typst가 YAML을 읽어 문서 표와 traceability 섹션을 만든다.
...
기준 데이터의 원본은 문서가 아니라 `examples/*/sot/*.yaml`에 있다.

---

## Agent Skills

AI Agent가 SoT와 Context의 경계를 지키며 작업하도록 skill을 둔다.

![Agent skills](images/agent-skills/cover.png)

- `swvnv-context-retrieval`  
  SoT 항목과 관련된 context evidence를 찾는다.

- `swvnv-document-drafting`  
  SoT를 기준으로, Context를 근거로 문서 초안과 수정안을 만든다.

- `swvnv-consistency-review`  
  SoT, Context, draft/generated documents 사이의 불일치와 누락을 찾는다.

- `swvnv-context-pdf-reader`  
  MFDS PDF context를 검색하고 필요한 페이지만 확인한다.
...
Context에서 SoT 변경 가능성이 발견되면 자동 반영하지 않는다.
...
- AI가 돕는 일  
  SoT/context retrieval, drafting, traceability gap review, open question 정리

- 사람이 판단하는 일  
  제품 의도, 위험 통제 충분성, 검증 타당성, SoT 변경 승인, 제출 가능 품질
...
SWVNV는 ALM, eQMS, RIM을 정면으로 대체하지 않는다.

![Positioning layer](images/positioning-layer/cover.png)

핵심 포지션은 SoT와 Context를 agent-readable하게 구성해  
인허가 문서 작성과 검토를 돕는 reasoning/document authoring layer다.
...
![SWVNV conclusion](images/swvnv-conclusion/cover.png)

SoT는 반복 정보의 기준을 제공하고, Context는 작성과 판단의 근거를 제공한다.  
Typst와 script는 기준 데이터를 문서에 재현 가능하게 탑재한다.  
Agent Skills는 AI가 이 경계를 지키며 작업하도록 만든다.
