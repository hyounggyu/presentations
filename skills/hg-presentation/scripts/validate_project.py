...
from project_config import audio_path, final_video_path, load_project, require_tools, timeline_path, voiceover_video_path


def validate(project_file: str, *, require_audio: bool = False, require_api_key: bool = True, require_ffmpeg: bool = True) -> int:
    errors: list[str] = []
    ...
    required_files = ["draft.md", "slides.md", "narration.json"]
    for name in required_files:
        path = project.presentation_dir / name
        if not path.exists():
            errors.append(f"Required project file does not exist: {path}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate an hg-presentation project.")
    parser.add_argument("project", help="Presentation directory or narration.json path")
    parser.add_argument("--require-audio", action="store_true", help="Require generated slide audio files.")
    parser.add_argument("--skip-api-key", action="store_true", help="Do not require ELEVENLABS_API_KEY or voice id.")
    parser.add_argument("--skip-ffmpeg", action="store_true", help="Do not require ffmpeg and ffprobe.")
