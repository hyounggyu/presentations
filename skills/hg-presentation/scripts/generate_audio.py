...
from project_config import Segment, Slide, load_project, segment_audio_path, silence_audio_path, slide_audio_path
...
def generate_segment_audio(api_key: str, voice_id: str, model_id: str, output_format: str, text: str, target: Path) -> None:
...
def generate(project_file: str, *, overwrite: bool = False) -> int:
    ...
    if not project.voice.output_format.startswith("mp3_"):
        print("ERROR: only mp3 ElevenLabs output formats are supported for segment concatenation", file=sys.stderr)
        return 1
    ...
    for slide in project.slides:
        ...
        for segment in slide.segments:
            ...
            if segment_target.exists() and not overwrite:
                ...
                try:
                    ...
                    print(f"ERROR: ElevenLabs request failed for slide {slide.index}, segment {segment.index}: {exc}", file=sys.stderr)
                    return 1
        ...
        try:
            subprocess.run(concat_command(concat_file, slide_target), check=True, capture_output=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate per-segment ElevenLabs TTS audio and slide-level MP3 files.")
