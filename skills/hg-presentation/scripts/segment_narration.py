...
from project_config import find_repo_root


SENTENCE_PATTERN = re.compile(r"[^.!?\n。！？]+[.!?。！？]?")
TAG_PATTERN = re.compile(r"^\s*(\[[^\]]+\]\s*)+")
FILE_DOT_PATTERN = re.compile(r"\b([A-Za-z0-9_-]+)\.(yaml|yml|json|toml|md|typ|py|js|ts|tsx|pdf|docx|pptx)\b", re.IGNORECASE)
FILE_DOT_TOKEN = "__HG_FILE_DOT__"
...
def split_sentences(text: str) -> list[str]:
    ...
    normalized = FILE_DOT_PATTERN.sub(lambda match: f"{match.group(1)}{FILE_DOT_TOKEN}{match.group(2)}", normalized)
...
def convert(raw: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for slide_index, slide in enumerate(slides_raw, start=1):
        ...
        for segment_index, chunk in enumerate(chunks, start=1):
            tag = choose_tag(chunk, slide_index=slide_index, segment_index=segment_index, slide_count=slide_count)
            segments.append(
                {
                    "text": tag_text(chunk, tag),
                    "pause_after_ms": pause_after(chunk, is_last_segment=segment_index == len(chunks)),
                }
            )
...
def run(project: str, *, dry_run: bool = False, force: bool = False) -> int:
    ...
    if backup_path.exists() and not force:
        print(f"Backup already exists: {backup_path}. Use --force to overwrite it.", file=sys.stderr)
        return 1

    shutil.copy2(narration_path, backup_path)
    narration_path.write_text(json.dumps(converted, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Convert legacy slides[].text narration to slides[].segments[].")
    parser.add_argument("project", help="Presentation directory or narration.json path")
    parser.add_argument("--dry-run", action="store_true", help="Print the segment plan without writing files.")
    parser.add_argument("--force", action="store_true", help="Overwrite narration.legacy.json if it already exists.")
