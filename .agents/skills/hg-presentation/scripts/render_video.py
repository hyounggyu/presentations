...
def render(project_file: str, *, overwrite: bool = False) -> int:
    ...
    if voiceover_path.exists() and not overwrite:
        print(f"ERROR: Voiceover video already exists: {voiceover_path}. Use --overwrite.", file=sys.stderr)
        return 1
    ...
    if final_path.exists() and not overwrite:
        print(f"ERROR: Export video already exists: {final_path}. Use --overwrite.", file=sys.stderr)
        return 1
